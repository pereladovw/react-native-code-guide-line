...
## <a name="#class-vs-reactcreateclass-vs-stateless">Class против компонента без состояния (stateless)</a>
    ...
    Если у вас нет состояния (`state`) или ссылок (`refs`), но есть вспомогательные методы, отдавайте предпочтение PureComponent:
    ...
    Если у вас нет состояния (`state`) или ссылок (`refs`) и вспомогательных методов, отдавайте предпочтение функциям над классами:
